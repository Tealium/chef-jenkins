...
template "/tmp/private_code/clone_update.sh"  do
   ...
   variables(
      ...
      :directory => "#{node[:jenkins][:jently]}"
   )
end

execute "/tmp/private_code/clone_update.sh" do
	action :run
end

template "#{node[:jenkins][:jently]}/config/config.yaml" do
	source "jently_config.yaml.erb"
	owner node[:jenkins][:server][:user]
  group node[:jenkins][:server][:group]
  mode 0700
  jently_config = search(:jently, "id:development").first
...
template "#{node[:jenkins][:jently]}/run_ruby_script.sh" do
...
execute "#{node[:jenkins][:jently]}/run_ruby_script.sh" do
	action :run
end
